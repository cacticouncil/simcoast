...
func update_month_labels():
	...
	if len(RainLevel.monthlyRainLevels) > 0:
		currentMonthInd = RainLevel.monthlyRainLevels[0][0]
...
func update_max():
	var m = 0
	for ind in range(len(RainLevel.monthlyRainLevels)):
		var point = RainLevel.monthlyRainLevels[ind]
		var val = point[1]
...
func draw_data():
	...
	if len(RainLevel.monthlyRainLevels) < 2:
		return 
		
	for ind in range(1, len(RainLevel.monthlyRainLevels)):
		# Prev point
		var y_1 = RainLevel.monthlyRainLevels[ind-1][1]
		
		var x_pos_1 = (ind) * x_spacing
		var y_pos_1 = rect_size.y - (y_1 * rect_size.y / float(max_value))
		
		# Current point
		var y_2 = RainLevel.monthlyRainLevels[ind][1]
