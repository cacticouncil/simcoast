...
func _tick(agent: Node, blackboard: Blackboard) -> bool:
	var current_agent = blackboard.get_data("queue2").pop_front()
			...
			if foundJob == false:
				...
				if current_tile.jobCapacity < current_tile.jobMax:
					if current_tile.has_utilities() && current_tile.tileDamage == 0:
						if current_tile.TileZone == Tile.TileZone.COMMERCIAL:
							foundJob = true
						...
						elif current_tile.TileZone == Tile.TileZone.PUBLIC_WORKS:
							if current_tile.TileInf == Tile.TileInf.PARK:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.PARK_RANGER
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.FIRE_STATION:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.FIREFIGHTER
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.HOSPITAL:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.HOSPITAL_EMPLOYEE
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.POLICE_STATION:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.POLICE_OFFICER
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.LIBRARY:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.LIBRARIAN
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.MUSEUM:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.MUSEUM_WORKER
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.SCHOOL:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.SCHOOL_EMPLOYEE
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.UTILITIES_PLANT:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.UTILITY_PLANT_OPERATOR
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.SEWAGE_FACILITY:
								foundJob = true
								current_tile.jobCapacity += 1
								current_agent.job = Agent.JOB_TYPES.SEWAGE_WORKER
								current_agent.change_job(current_tile)
								current_agent.job_level()
							elif current_tile.TileInf == Tile.TileInf.WASTE_TREATMENT:
								foundJob = true
								current_tile.jobCapacity += 1
...
func check_empty(blackboard: Blackboard) -> void:
	var empty = blackboard.get_data("queue2").empty()
	print(str("empty",empty))
	if empty:
		blackboard.set_data("queue2_empty", true)
