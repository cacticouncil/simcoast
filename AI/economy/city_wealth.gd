...
#		tile.wealth_weight = 4
		...
		for y in Global.mapWidth:
			if is_valid_tile(x, y):
				...
				if currTile.is_commercial():
					currTile.profitRate = (avg_income_around_tile(tile.i, tile.j) * Econ.SALES_TAX) * \
	...
	match tile.get_zone():
		Tile.TileZone.LIGHT_COMMERCIAL:
			...
			elif tile.profitRate < avg_profit - 1000:
				tile.wealth_weight = -1
			#mild positive influence if tile is around the average
			elif avg_profit - 1000 < tile.profitRate && tile.profitRate < avg_profit + 1000:
				tile.wealth_weight = 1
		...
		Tile.TileZone.LIGHT_RESIDENTIAL:
			if tile.profitRate < 0:
				tile.wealth_weight = -2
			elif tile.profitRate < avg_income - 1000:
				tile.wealth_weight = -1
			elif avg_income - 1000 < tile.profitRate && tile.profitRate < avg_income + 1000:
				tile.wealth_weight = 1
			else:
				tile.wealth_weight = 2 
